...
func (pq *PageQuery) QueryComments() *CommentQuery {
...
func (pq *PageQuery) WithComments(opts ...func(*CommentQuery)) *PageQuery {
	query := (&CommentClient{config: pq.config}).Query()
...
func (pq *PageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Page, error) {
	...
	if query := pq.withComments; query != nil {
		if err := pq.loadComments(ctx, query, nodes,
			func(n *Page) { n.Edges.Comments = []*Comment{} },
			func(n *Page, e *Comment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PageQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Page, init func(*Page), assign func(*Page, *Comment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Page)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(comment.FieldPageID)
	}
	query.Where(predicate.Comment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(page.CommentsColumn), fks...))
	}))
