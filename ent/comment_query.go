...
func (cq *CommentQuery) QueryPage() *PageQuery {
	query := (&PageClient{config: cq.config}).Query()
...
func (cq *CommentQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (cq *CommentQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (cq *CommentQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (cq *CommentQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (cq *CommentQuery) IDs(ctx context.Context) (ids []xid.ID, err error) {
...
func (cq *CommentQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CommentQuery) WithPage(opts ...func(*PageQuery)) *CommentQuery {
	query := (&PageClient{config: cq.config}).Query()
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withPage; query != nil {
		if err := cq.loadPage(ctx, query, nodes, nil,
			func(n *Comment, e *Page) { n.Edges.Page = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CommentQuery) loadPage(ctx context.Context, query *PageQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Page)) error {
...
func (cq *CommentQuery) loadReplyTo(ctx context.Context, query *CommentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Comment)) error {
	...
	nodeids := make(map[xid.ID][]*Comment)
	for i := range nodes {
		if nodes[i].ReplyToID == nil {
			continue
		}
		fk := *nodes[i].ReplyToID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "reply_to_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CommentQuery) loadReplies(ctx context.Context, query *CommentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Comment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(comment.FieldReplyToID)
	}
	...
	for _, n := range neighbors {
		fk := n.ReplyToID
		if fk == nil {
			return fmt.Errorf(`foreign-key "reply_to_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "reply_to_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withPage != nil {
			_spec.Node.AddColumnOnce(comment.FieldPageID)
		}
		if cq.withUser != nil {
			_spec.Node.AddColumnOnce(comment.FieldUserID)
		}
		if cq.withReplyTo != nil {
			_spec.Node.AddColumnOnce(comment.FieldReplyToID)
		}
	}
